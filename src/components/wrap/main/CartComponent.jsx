...
export default function CartComponent({cartKey}){
    ...
    const onClickSub=(e, record)=>{
        ...
        const result = cart.map((item)=>{
            return(
                item.상품코드 === record.상품코드 ? ({...item, 수량: (item.수량>=2 ? item.수량-1 : 1), 총결제금액 : Math.round((item.수량>=2 ? item.수량-1 : 1)*(item.가격*(1-item.할인율))) }) : ({...item})
            )
        });
        setCart(result);
    ...
    const onClickAdd=(e, record)=>{
        ...
        const result = cart.map((item)=>{
            return(
                item.상품코드 === record.상품코드 ? ({...item, 수량: (item.수량+1), 총결제금액 : Math.round((item.수량+1)*(item.가격*(1-item.할인율))) }) : ({...item})
            )
        });
        setCart(result);
        localStorage.setItem('ABCMARTCART', JSON.stringify(result));
    ...
    const onClickDel=(e, record)=>{
        ...
        // console.log(record);
        if(confirm('삭제하시겠습니까?')){
            const result = cart.filter((item)=>item.상품코드!==record.상품코드);
            setCart(result);
            localStorage.setItem('ABCMARTCART', JSON.stringify(result));
            initMethod();
        }
    ...
    const onClickSelectDelete=(e)=>{
        ...
        const result = cart.filter((item)=> !check.includes(item.상품코드));
        setCheck([]);
        setCart(result);
        localStorage.setItem('ABCMARTCART', JSON.stringify(result));
        initMethod();
                                        ...
                                        <div className="cart-box">
                                            <div className="cart-top-box">
                                                <span className='all-select-btn'>
                                                    <input type="checkbox" id='allSelect' checked={check.length===cart.length} onClick={onClickSelectAll} />
                                                                        ...
                                                                        <tr key={idx}>
                                                                            <td className='col1'>
                                                                                <span className='select-btn'>
                                                                                    <input onChange={onChangeCheck} checked={check.includes(item.상품코드)} type="checkbox" name='select' id='select1' value={item.상품코드} />
                                                                                </span>
                                                                            </td>
                                                                            <td className='col2'>
